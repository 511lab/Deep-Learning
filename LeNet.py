# -*- coding: utf-8 -*-
"""
Created on Sun Oct  8 11:03:38 2023
...
def train_data_process():
    # 加载FashionMNIST数据集
    train_data=torchvision.datasets.FashionMNIST(root=r'D:\任务\深度聚类'
                                        ,download=False # 不选择下载
                                        ,train=True # 选用train数据集
                                        ,transform=transforms.ToTensor()# 转化成tensor以便后面方便操作
                                        # ,transform=transforms.Compose([transforms.Resize(size=224), transforms.ToTensor()])  
                                        # 把PIL.Image或者numpy.array数据类型转变为torch.FloatTensor类型

                                       )  # FashionMNIST上一层目录
    train_loader = Data.DataLoader(dataset=train_data,  # 传入的数据集
                                   batch_size=64,  # 每个Batch中含有的样本数量 64
    ...
    for ii in np.arange(len(batch_y)):
        plt.subplot(4, 16, ii+1)  # 这里需要调整，根据每个batch的大小调整
        plt.imshow(batch_x[ii, :, :], cmap=plt.cm.gray)
        plt.title(class_label[batch_y[ii]], size=8)
        plt.axis("off") # 去掉坐标轴
        plt.subplots_adjust(wspace=0.05) # 子图间隔
    plt.show()
...
def test_data_process():
    test_data = FashionMNIST(root=r'D:\任务\深度聚类',  # 数据路径
...
# 网络模型
class wyw_LeNet5(nn.Module):
    def __init__(self):
        super().__init__()
        self.features_=nn.Sequential(nn.Conv2d(1,6,kernel_size=5),nn.ReLU(inplace=True)
                                    ,nn.AvgPool2d(kernel_size=2,stride=2)
                                    ,nn.Conv2d(6,16,kernel_size=5),nn.ReLU(inplace=True)
                                    ,nn.AvgPool2d(kernel_size=2,stride=2)
                                   )
        self.classifier=nn.Sequential(nn.Linear(16*4*4,4096),nn.ReLU(inplace=True)
...
def train_model_process(myconvnet):
    ...
    myconvnet, train_process = train_model(myconvnet, train_loader, 0.8, criterion, device, optimizer, num_epochs=20)  # 开始训练模型
    PRE,PRV=test_model(myconvnet, test_loader, device)  # 使用测试集进行评估
    
    conf_mat=confusion_matrix(PRE,PRV)
...
if __name__ == '__main__':
    ratio = 8
    wyw_LeNet5=wyw_LeNet5()
    train_model_process(wyw_LeNet5)